...
/*
   *
   *
   * Returns. the 4dim array of asymmetric coulomb elements. <ij||kj>
   *
   *
   */
double**** hamiltonianElem::initCoulombElem()
{
	double ****coulelem;
	int i,j,k,l;
	
	coulelem = new double***[i_numstates];
	for (i=0; i<i_numstates; i++)
	{
		coulelem[i] = new double**[i_numstates];
		for (j=0; j<i_numstates; j++)
		{
			coulelem[i][j] = new double*[i_numstates];
			for (k=0; k<i_numstates; k++)
			{
				coulelem[i][j][k] = new double[i_numstates];
			}
		}
	}
		...
		for (j=0; j<i_numstates; j++)
			for (k=0; k<i_numstates; k++)
				for (l=0; l<i_numstates; l++)
				{
					...
					if (fabs(coulelem[i][j][k][l])<1e-15)
					{
						coulelem[i][j][k][l] = 0;
					}
					//if (coulelem[i][j][k][l] != 0)
					//if ((i==9)&&(j==2)&&(k==9)&&(l==2))
					//{
					//std::cout << "\n" << i << "," << j << "," << k << "," << l << ":"
					//	   	 <<	coulelem[i][j][k][l] << "\n";
